...
export class PaginationHelper {
	...
	public pageItemCount(pageIndex: number) {
        return pageIndex < this.pageCount() - 1 ? this.itemsPerPage : this.collection.length % this.itemsPerPage;
	}
	public pageIndex(itemIndex: number) {
        return itemIndex < 0 || itemIndex >= this.collection.length ? -1 : Math.floor(itemIndex / this.itemsPerPage);
	}
}
