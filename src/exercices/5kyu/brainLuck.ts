...
export function initBrainLuck() {
	return brainLuck(",+[-.,+]", "Codewars" + String.fromCharCode(255));
}

export function brainLuck(code: string, input: string) {
	const instructions = code.split("");
	const memory = input.split("").map((char) => char.charCodeAt(0));
	let pointer = 0;
	const maxByte = 255;
	const minByte = 0;
	const posibilities: { [key: string]: (...args: any) => any } = {
		">": () => pointer++,
		"<": () => pointer--,
		"+": (value: number) => {
			value++;
			return value > maxByte ? minByte : value;
		},
		"-": (value: number) => {
			value--;
			return value < minByte ? maxByte : value;
		},
		"[": () => {},
		"]": () => {},
		".": () => {},
	...
	do {
		...
		const func = posibilities[instruction];
		const bytes = memory[pointer];
		func(bytes);
        pointer++;
	} while (instructions[pointer] !== ".");

	return memory.map((byte) => String.fromCharCode(byte)).join("");
